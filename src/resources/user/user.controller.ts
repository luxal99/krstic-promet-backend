import {Body, Controller, HttpStatus, Post, Put, Req, Res} from "@nestjs/common";
...
export class UserController extends GenericController<User> {
    ...
    async changePassword(@Req() req: Request, @Res() res: Response) {
        try {
            const username = await jwt.verify(req.headers.authorization, process.env.TOKEN_SECRET).username;
            const userByName = await this.userService.findByUsername(username);
            const body: ChangePasswordDto = req.body;
            const isPasswordMatching = await bcrypt.compare(body.oldPassword, userByName.password);
            if (isPasswordMatching && body.password === body.passwordConfirm) {
                ...
                res.status(HttpStatus.BAD_REQUEST).send({message: "Šifre se ne poklapaju"});
            }
        } catch (err) {
            res.send({err});
        }
